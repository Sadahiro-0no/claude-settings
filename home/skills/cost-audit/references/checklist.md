# コスト監査チェックリスト

各項目: **確認方法 → 問題パターン → 推奨修正 → 期待効果**。
重要度の目安: Critical = 恒常的に数十%規模の無駄 / High = 明確な無駄 / Medium = 積み上げ・予防。

---

## A. モデル選択

### A-1. メインモデルが Opus のまま [Critical]
- 確認: `ANTHROPIC_MODEL` / settings の `model`
- 問題: Opus は Sonnet の約 1.7 倍(入力 $5 vs $3 / 1M)、出力も $25 vs $15。日常のコーディングは Sonnet で十分な品質。
- 修正: メインを Sonnet 系に。Opus は難タスク時に `/model` で一時切替する運用に。
- 効果: 単価ベースで約 40% 減。

### A-2. バックグラウンドモデル未指定 [High]
- 確認: `ANTHROPIC_DEFAULT_HAIKU_MODEL`(旧 `ANTHROPIC_SMALL_FAST_MODEL`)
- 問題: 会話タイトル生成などの補助呼び出しが高価なモデルで動くよう上書きされている、または利用環境によっては明示指定しないと解決されない。
- 修正: Haiku 系のモデルを指定する。利用環境固有のモデル ID 形式が必要な場合は、動作中のメインモデル ID と同形式・同プレフィックスの Haiku ID を使う。未確認のまま仮の ID を設定してはいけない(モデル解決エラーになる)。ID が確認できない間は未設定のままにする(他の削減策は機能する)。
- 効果: 補助呼び出しが 1/3〜1/5 の単価に。

### A-3. サブエージェントのモデル未指定 [High]
- 確認: `~/.claude/agents/*.md`, `.claude/agents/*.md` の frontmatter `model`、`CLAUDE_CODE_SUBAGENT_MODEL`
- 問題: 探索・調査系サブエージェントがメインモデルを継承している。探索はトークン量が多く、Haiku で品質がほぼ落ちない代表例。
- 修正: 読み取り専用エージェントに `model: haiku` を明示。
- 効果: 探索コスト約 1/3。

### A-4. 補助的なモデル呼び出しの抑制が未設定 [Medium]
- 確認: env に `CLAUDE_CODE_DISABLE_TERMINAL_TITLE` があるか。旧 `DISABLE_NON_ESSENTIAL_MODEL_CALLS` は現行ドキュメントから削除されており、設定されていても機能する保証がない(仕様ドリフト)。
- 修正: `"CLAUDE_CODE_DISABLE_TERMINAL_TITLE": "1"` — 会話内容からの端末タイトル自動生成(ヘッドレス実行ではバックグラウンドの Haiku 呼び出し)を止める。旧変数が残っていたら置き換えを提案。あわせて `"CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC": "1"`(テレメトリ等の非本質通信の一括停止)も確認。

---

## B. プロンプトキャッシュ

キャッシュは**プレフィックス一致**。先頭側の1バイトの変化で以降すべてが無効化される。ヒット部は約 0.1 倍。書き込みは TTL 依存: 既定の5分TTLで 1.25 倍、1時間TTLオプションで 2 倍。

### B-1. キャッシュ無効化フラグ [Critical]
- 確認: env に `DISABLE_PROMPT_CACHING` / `CLAUDE_CODE_DISABLE_PROMPT_CACHING` 類が入っていないか
- 問題: 過去のトラブルシュートの残骸で無効化されたまま、というのが典型。全リクエストがフルプライスになる。
- 修正: 削除。

### B-2. 動的コンテンツがプレフィックスに混入 [Critical]
- 確認: CLAUDE.md や SessionStart/UserPromptSubmit フックが、日時・ブランチ名・`git status` 出力などの**毎回変わる文字列**をコンテキスト先頭側に注入していないか
- 問題: 毎ターン、キャッシュが先頭から無効化される。「設定は正しいのにキャッシュヒットしない」場合の筆頭原因。
- 修正: 動的情報の注入をやめる。必要ならユーザーが都度プロンプトで渡す。
- 効果: キャッシュヒット率の回復(実効入力単価が最大 10 倍変わる)。

### B-3. セッション途中の MCP / モデル切替の常態化 [Medium]
- 問題: ツール定義とモデルはプレフィックス最先頭。途中で変えるとキャッシュ全滅。
- 修正: MCP 構成はセッション開始前に確定させる運用を CLAUDE.md 等に明記。

### B-4. 5分以上の放置が多い作業スタイル [Medium]
- 問題: キャッシュ TTL(既定5分)切れで再ウォーム(書き込み係数 1.25〜2倍)が頻発。
- 修正: 運用の話としてレポートに記載(長時間離席するならセッションを畳む)。

---

## C. コンテキスト最小化

### C-1. CLAUDE.md の肥大化 [High]
- 確認: `estimate_tokens.sh` で計測。目安: グローバル+プロジェクト合計で **2,000 トークン以下**を推奨、5,000 超は要ダイエット。
- 問題: CLAUDE.md は**全リクエストの入力**に乗る固定費。キャッシュが効いても 0.1 倍 × 毎回は積み上がる。
- 修正: 「常に必要な規範」だけ残し、手順書・仕様・稀にしか使わない知識は**スキル(オンデマンド読み込み)へ移す**。特定のファイル群にだけ適用する規約は **path-scoped rules**(`.claude/rules/*.md` の frontmatter `paths`)へ — 該当ファイルを扱うときのみ読み込まれる公式機構で、常駐しない。@import の常用は同罪(import 先も起動時に毎回読み込まれる — 公式仕様)。

### C-2. MCP サーバーの過剰接続 [High]
- 確認: `claude mcp list`、`.mcp.json`、`~/.claude.json`
- 問題: 接続中の全サーバーの全ツール定義が毎リクエストの入力になる。使っていないサーバーが最悪。
- 修正: 使うプロジェクトだけ `.mcp.json` で有効化。グローバル常時接続をやめ、`disabledMcpjsonServers` で絞る。
- 効果: 1サーバーあたり数百〜数千トークン × 毎リクエスト。

### C-3. permissions.deny の不足 [High]
- 確認: deny に node_modules / dist / ロックファイル / *.min.js / .env 系があるか
- 問題: 誤って読むと1ファイルで数万トークン消費し、さらに**以降の全ターンでその内容がコンテキストに残り続ける**。
- 修正: 本リポジトリ `home/settings.json` の deny リストを適用。Bash 経由(`cat` 等)は `guard-heavy-read.sh` フックで補完。

### C-4. /clear・/compact の運用不在 [Medium]
- 確認: CLAUDE.md にセッション運用ルールがあるか
- 問題: 無関係な前タスクの文脈を引きずると、以降の全ターンでその分を(キャッシュ済みでも 0.1 倍で)払い続ける。
- 修正: 「タスクが変わったら /clear」をグローバル CLAUDE.md に明記。

### C-5. 探索をメインコンテキストでやっている [High]
- 確認: 読み取り専用の探索エージェント(例: `explore`)が定義され、CLAUDE.md から委譲が指示されているか
- 問題: 横断調査の中間結果(大量のファイル内容)がメインの文脈に堆積し、以降ずっと課金される。
- 修正: `agents/explore.md` を導入し、委譲ルールを CLAUDE.md に記載。

### C-6. 大出力の無加工取り込み [High]
- 確認: CLAUDE.md・スキル・エージェント定義に、テスト/ビルドの実行やWeb取得の「取り込み方」の規範があるか。トランスクリプトに全量ログ(数千行のテスト出力等)が頻出していないか。
- 問題: 一度コンテキストに入ったツール出力・Web結果・画像は**以降の全ターンで再送される**。テストログ全量1回 = 数千〜数万トークンの恒久負債。
- 修正: 規範を明文化する — テストは対象を絞って実行、長い出力は `tail`/`grep`/`--quiet` で要点のみ、Web取得は必要部分の抽出を指示、スクリーンショットは必要最小限。大量ログの精査が必要なときはファイルに落として explore サブエージェントに読ませる。

### C-7. 手戻りループ(プロンプト品質) [High]
- 確認: CLAUDE.md に「曖昧な大きい依頼は探索前に確認する」規範があるか。実行前整形の手段(`/refine` 相当のスキル)が存在するか。トランスクリプトに「探索→実装→やり直し」のパターンが頻出していないか。
- 問題: 曖昧な依頼は 投機的探索(数千〜数万トークン)+ やり直し + **誤った文脈の残骸が以降の全ターンで再送**という三重コストを生む。設定側の最適化では消せない唯一の大物。
- 修正: 確認ファーストの規範を CLAUDE.md に明記(自動)。大きめの依頼は `/refine` で 目的/範囲/制約/完了条件 を仕様化してから着手(半自動)。1往復の確認(数百トークン)は手戻りより2桁安い。

---

## D. thinking / 出力

### D-1. 常時 thinking [High]
- 確認: settings の `alwaysThinkingEnabled`、env の `MAX_THINKING_TOKENS`
- 問題: 出力トークンは入力の5倍単価。全ターン thinking は簡単なタスクで純粋な無駄。
- 修正: `alwaysThinkingEnabled: false`。難タスクのみプロンプトで思考を要求する運用。

### D-2. 出力の冗長性を抑えるルールがない [Medium]
- 確認: CLAUDE.md に「結論先行・変更箇所のみ提示・全文再掲禁止」があるか
- 修正: グローバル CLAUDE.md に3行で記載(本リポジトリの `home/rules/cost-optimization.md` 参照)。

### D-2b. 出力スタイル・追加システムプロンプトの常駐コスト [Medium]
- 確認: 有効な出力スタイル(`/output-style`、settings の `outputStyle`)と `--append-system-prompt` の内容。スタイル本文は**有効な間、毎リクエストのシステムプロンプトに常駐**する。
- 問題: 長大なカスタムスタイルは固定費。逆に、説明不要の定型作業フェーズで既定スタイルのままだと地の文が無駄。
- 修正: スタイル本文は300トークン程度までに保つ。定型作業の連続時のみ `/output-style terse` に切替え、フェーズが終わったら default に戻す(terse の節約は数%規模なので常用の価値はない。成果物・エラー説明の品質維持が条件)。

### D-3. `CLAUDE_CODE_MAX_OUTPUT_TOKENS` の極端な値 [Medium]
- 問題: 大きすぎは暴走時の上限がない、小さすぎは途中切断→リトライで逆に高くつく。
- 修正: 基本は未設定(デフォルト)。設定するなら 16000〜32000。

---

## E. 可視化・運用

### E-1. コストの可視化がない [Medium]
- 確認: `statusLine` 設定、OTEL テレメトリ
- 修正: `statusline.sh` 導入。チーム運用なら `CLAUDE_CODE_ENABLE_TELEMETRY=1` + OTLP エクスポート(docs/cost-optimization.md §7)。

### E-1c. statusLine.refreshInterval の欠落 [Low]
- 確認: `settings.json` の `statusLine.refreshInterval` が設定されているか(`jq '.statusLine.refreshInterval'`)。
- 問題: Claude Code は statusline を「新しいアシスタントメッセージの後」等のイベント時にのみ再実行する。`/model` でのモデル切替やパーミッションモード変更はこのイベントに含まれないため、`refreshInterval` が無いと**次に応答が返るまで表示が古いまま**になる(利用者から見ると「モデルを切り替えたのに statusline が変わらない」というバグに見える)。
- 修正: `statusLine.refreshInterval: 5`(秒)を設定する。statusline はローカル実行でAPIトークンを消費しないため、コスト面のデメリットはない。

### E-1b. セッション予算ガードの不在・閾値の形骸化 [High]
- 確認: hooks に `session-budget-guard.sh`(PreToolUse `*` + UserPromptSubmit)が配線されているか。env の `CLAUDE_SESSION_BUDGET_USD` が実態に合っているか(平均セッションコストの2〜3倍が目安。高すぎると発火せず形骸化、低すぎると日常作業が中断される)。`CLAUDE_TURN_BUDGET_USD`(目標ペース、既定 $0.10/ターン = 10ターン≒$1)も直近の実績ペース(statusline の `10T≈` 表示)と照合して較正する。`CLAUDE_CTX_LIMIT_TOKENS`(コンテキスト肥大の介入閾値、既定 12万)は statusline の `ctx:` 表示の典型値の2倍程度が目安。`CLAUDE_TURN_HARD_LIMIT`(ターン数上限、既定 10。コストと独立の第2軸)は典型セッションのターン数に対して「1タスクを畳める長さ」に較正する — 頻繁に上限で中断されるなら値を上げるか 0 で無効化し、コスト軸のみで運用する。
- 問題: ガードがないと「気づいたら高額セッション」を止める手段がモデルの自制しかない。
- 修正: フックを配線し、直近の `/cost` 実績から閾値を較正する。

### E-2. 設定の二重定義 [Medium]
- 確認: 同じ変数が シェル環境 / settings.json(user, project, local) / managed に重複していないか
- 問題: 優先順位(managed > CLI > local > project > user)を誤解し、「設定したのに効かない」→ 誤った方向のチューニングへ。
- 修正: 定義箇所を一本化(推奨: ユーザー共通は `~/.claude/settings.json`、プロジェクト固有は `.claude/settings.json`)。

### E-2b. 常駐メモリによる再開運用 [Medium]
- 確認: CLAUDE.md やメモリファイルに「前回作業の続き」系の内容が恒常的に追記されていないか。SessionStart 系フックが大きなコンテキストを毎セッション注入していないか。
- 問題: 再開用メモの常駐化は毎リクエストの固定費になり、肥大化も進む。
- 修正: ハンドオフ方式へ移行(`.claude/handoff.md` + `handoff-notice.sh`。docs/cost-optimization.md §6)。CLAUDE.md に溜まった作業メモは削除する。

### E-3. autocompact を無効化している [Medium]
- 問題: コンテキスト溢れ間際の劣化・やり直しの方が高くつく。
- 修正: 有効のまま。長いタスクの区切りで手動 `/compact` も可。

---

## F. スキル資産(既存スキルの見直し)

**大原則: スキルの目的・期待される出力品質を落とす修正はしない。** 最適化するのは「構造」(どこに何を置くか)であり「能力」ではない。スキルの削除・統合は必ずユーザーの承認を取る。

### F-1. 常駐 description の総量 [High]
- 確認: 全スキル(`~/.claude/skills/*`, `.claude/skills/*`, プラグイン由来)と**全エージェント**(`agents/*.md`)の frontmatter `description` を列挙し、合計トークンを見積る。**description は全セッションに常駐する**(本文はオンデマンド)。
- 問題: スキルが増えるほど常駐コストが線形に増える。1つ200トークンの description ×20個 = 4,000トークンの固定費。
- 修正: description は「いつ発火すべきか」が判別できる最小限(50〜80トークン目安)に圧縮。発火条件のキーワードは残す(削りすぎると発火しなくなり、スキルの価値自体が失われる — 品質側の失敗)。

### F-2. SKILL.md 本文の肥大化 [High]
- 確認: 各 SKILL.md 本文のトークン量(`estimate_tokens.sh`)。目安: 本文 1,000 トークン超は分割候補。
  ただし判定は**「詳細が references に分離されているか」**で行う。分離済みで本文が手順の骨格中心なら、
  トークン数が目安を超えていても適正とみなす(単純な行数・トークン数での機械的判定はしない)。
- **自己参照の除外**: cost-audit スキル自身は監査対象から除外する(または「分離済み・適正」と注記して
  スキップする)。監査手順を持つスキルは本質的に骨格が長くなるため、自分を肥大化として過剰検知しない。
- 問題: 本文は発火のたびに全文読み込まれる。滅多に使わない詳細・網羅表・長い例が本文にあると、発火毎にその分を払う。
- 修正: 本文には「手順の骨格と判断基準」だけを残し、詳細は `references/*.md` へ(必要時のみ読まれる)。**内容は移動であって削除ではない** — スキルの網羅性・正確性は維持する。

### F-3. モデルにやらせている決定論的処理 [High]
- 確認: スキル本文に「数える」「集計する」「差分を取る」「フォーマット変換する」等、計算可能な作業をモデルに指示している箇所がないか。
- 問題: 決定論的処理をモデルにやらせると、トークンを消費する上に不正確。
- 修正: `scripts/*.sh`(または該当言語)に切り出し、スキルは「スクリプトを実行して結果を使う」指示に変える。品質はむしろ向上する(正確・再現可能になる)。

### F-4. 誤発火・重複スキル [Medium]
- 確認: description が曖昧で無関係なタスクでも発火しうるスキル、内容が CLAUDE.md や他スキルと重複するスキル。
- 問題: 誤発火1回 = 本文読み込み+無関係な指示の混入(コストと品質の両方を損なう)。
- 修正: description に発火条件(Use when ...)を明記。手動起動専用のワークフローには frontmatter `disable-model-invocation: true` を設定(自動発火を仕様として封じる)。重複は片方に寄せる(ユーザー承認の上で)。

### F-5. スキル単位の model / effort / fork 未活用 [Medium]
- 確認: 各 SKILL.md の frontmatter に `model` / `effort` / `context` の指定があるか。定型・機械的なスキル(フォーマット変換、チェックリスト消化、テンプレート出力)が高価なモデル・高 effort のまま動いていないか。
- 問題: スキル実行中のモデル・effort はセッション設定を継承する。判断の軽いスキルにメインと同じ単価を払うのは A-3(サブエージェント)と同型の無駄。
- 修正: 軽いスキルに `model: haiku` や `effort: low` を指定(そのターンのみ有効・設定は汚さない)。大量読み込みを伴う読み取り専用スキルは `context: fork` でメイン文脈への堆積を防ぐ(ただし対話が必要なスキルには不向き)。品質が要るスキルには指定しない — 単価より手戻りの方が高い。

---

## G. バックグラウンドのトークン消費

`scripts/scan_background.sh` を実行して機械的に検出する(読み取り専用)。

### G-1. 放置プロセス・多重起動 [High]
- 確認: スキャナの「実行中の claude プロセス」。長時間の etime を持つヘッドレス実行(`-p`/`--print`)、見覚えのない多重起動。
- 問題: 自動リトライやループに入ったヘッドレスプロセスは無人でトークンを消費し続ける。
- 修正: 不明なプロセスは kill。定常的に必要なものは呼び出しモデル・頻度・上限を明示した設計に直す。

### G-2. cron / タイマー / CI からの自動起動 [High]
- 確認: スキャナの crontab / systemd / launchd 検出。CI(GitHub Actions 等)で claude を呼ぶワークフローがあれば実行頻度と1回あたりの消費も確認。
- 問題: 「毎時レポート生成」のような自動化は、1回が安くても月間では大きい(例: 1回$0.2×毎時=月$144)。
- 修正: 頻度を必要最小限に。定型部分は決定論的スクリプトに寄せ、モデル呼び出しは要約等の最小範囲+haiku 系に。

### G-3. フックからのモデル呼び出し [Critical]
- 確認: スキャナの「フック内からのモデル呼び出し」。フック(PreToolUse 等)から `claude -p` を呼んでいる箇所。
- 問題: ツール呼び出しやプロンプト送信の**たびに**追加のモデル課金が発生する。1セッションで数百回発火しうる。
- 修正: フックは決定論的スクリプト(jq/grep/独自ロジック)に限定する。本リポジトリのフックがすべて非モデルなのはこのため。

---

## H. 仕様ドリフト(Claude Code 本体の変化への追従)

ガード類はフェイルオープン設計(異常時は作業を止めない)のため、Claude Code の更新で
フック入力やトランスクリプトの形式が変わると**静かに無効化**されうる。三重の検知で表面化させる:
①ガード自身のランタイム検知(読めない入力・集計ゼロのセッションを1回だけ警告)、
②カナリア自己診断(下記 H-1)、③SessionStart での本体更新検知(下記 H-2)。

### H-1. 自己診断の失敗 [Critical]
- 確認: `bash ~/.claude/skills/cost-audit/scripts/selftest_guard.sh` を**監査のたびに必ず実行**。
  既知データでのコスト集計一致・上限ブロック作動・実トランスクリプトの解釈可否を検証する。
- 問題: 自己診断が NG ならコスト保護は機能していない(見かけ上は何も起きないのが危険)。
- 修正: NG の場合はレポートの先頭で報告し、claude-settings リポジトリの更新を確認・適用する。
  更新がない場合は、実トランスクリプトの新形式を調査してガードの解析ロジックの修正を提案する。

### H-2. Claude Code 更新時の再監査 [Medium]
- 確認: SessionStart フックが本体実行ファイルの更新を自動検知し、再監査を促す通知を出す。
  新しい設定キー・ディレクトリはフェーズ0の「未分類」として捕捉される。
- 修正: 更新通知が出たら /cost-audit を実行し、H-1 の自己診断結果と未分類項目を確認する。

---

## I. 設定の競合(既存資産との衝突)

複数階層・複数ファイルの設定は競合しうる。**解決ルールは面ごとに異なる**ため、競合の有無だけでなく
「どちらが勝つか/両方走るか/モデル判断に委ねられるか」を確認する。解決ルール表は
docs/cost-optimization.md「設定競合の解決ルール」を参照。

### I-1. hooks の相互作用(累積実行)[High]
- 確認: 同一イベント(PreToolUse 等)に複数のフックが登録されていないか。登録されている場合、
  それぞれが exit 2(ブロック)する条件と、モデル呼び出し(G-3)の有無。
- 問題: hooks は上書きではなく**累積**(全部走る)。①どれか1つでもブロックすれば実行は止まる
  (最も制限的なものが勝つ)ため、ユーザーの意図した操作が別フックに阻まれることがある。
  ②他フックがモデル呼び出しをしていると、予算ガードのコスト計算の**外側**で消費が起きる。
- 修正: フック同士の意図が衝突していないか確認。モデル呼び出しフックは G-3 に従い決定論化。
  install.sh は hooks を新旧和集合でマージ(既存フックは保持)するので、導入で消えることはない。

### I-2. settings スカラ値の上書き競合 [Medium]
- 確認: 同じキー(model, alwaysThinkingEnabled, env の各変数)が user/project/local/managed の
  複数階層にあるか(E-2 の二重定義と連動)。
- 問題: スカラ値は優先順位(managed > CLI > local > project > user)で**勝敗が決まる**。
  意図せぬ階層の値が有効になり「設定したのに効かない」→ 誤った方向の調整を招く。
- 修正: 定義箇所を一本化。install.sh の保持マージは同名キーをテンプレート優先で上書きするため、
  ユーザーが保持したい値(独自の閾値等)はマージ後に確認する。

### I-3. CLAUDE.md / スキル指示の矛盾 [Medium]
- 確認: グローバルとプロジェクトの CLAUDE.md、発火中のスキル本文の間で、相反する指示
  (例:「常に簡潔に」対「必ず詳細な手順を書く」)がないか。
- 問題: CLAUDE.md は階層が**連結**され、矛盾はモデルの解釈で解決される=**非決定的**。
  出力がぶれ、再試行(=追加コスト)や品質不安定の原因になる。
- 修正: 矛盾する指示は一方に寄せる。スコープが違う場合は適用条件を明示
  (「テスト作成時のみ詳細に」等)。出力スタイルは1つしか有効にならない(排他)点にも注意。
