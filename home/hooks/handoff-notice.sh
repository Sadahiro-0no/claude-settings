...
EOF
  ...
  echo ""
  echo '--- handoff ここまで ---'
  echo "ユーザーの依頼がこの続きに関係する場合は、上記の内容だけで直ちに再開すること(このファイルを改めて Read しない)。無関係な依頼なら一切言及せず無視してよい。"
  echo "再開時の注意: この handoff は前回終了時点のスナップショット。着手前に『現在地』と git status(あれば)を突き合わせ、実際の状態がメモとズレていないか一度だけ確認してから進めること。引き継いだ作業が完了したら $handoff を削除すること。"
fi
exit 0
