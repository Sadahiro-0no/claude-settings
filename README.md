...
1. `install.sh` が `home/` の内容を `~/.claude/` に導入します(既存ファイルは `~/.claude/backup-<日時>/` に退避)。**あなたが編集したファイルは上書きしません** — フック・statusline・スキルは「マニフェスト方式」で管理し、あなたが手を加えていないファイルだけ自動更新、編集したファイルは保持してテンプレ新版を `<file>.claude-settings-new` として隣に置きます(差分を見て必要なものだけ反映。全部テンプレに合わせるなら `CLAUDE_INSTALL_FORCE=1`)。`settings.json` と `CLAUDE.md` はさらに専用マージで扱います(下記)。
2. **`CLAUDE.md`(グローバルメモリ)は丸ごと上書きしません**。テンプレのコスト方針を `<!-- >>> claude-settings managed >>> -->` … `<!-- <<< claude-settings managed <<< -->` で囲んだ**管理ブロック**として扱い、更新時はそのブロック内だけを最新化します。ブロック外にあなたが書いた内容は保持され、あなた独自の `CLAUDE.md` があればブロックを末尾に追加するだけです(不要ならそのブロックを削除すればOK)。
...
- `CLAUDE.md` は**管理ブロック内だけ最新化**します。ブロック外にあなたが書いた内容は保持され、同一なら「変更なし・スキップ」。あなたの記述が丸ごと上書きされることはありません。
