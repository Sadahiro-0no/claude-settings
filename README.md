...
| `handoff-autostub.sh` | `SessionEnd`(`/clear` 時のみ) | handoff の書き忘れ | モデル未関与の決定論的スタブ(日時・ディレクトリ・gitブランチ・**作業中ファイル(`git status`)・直近コミット**・直近の依頼・元トランスクリプトへのポインタ)を**セッション別** `.claude/handoff-<id>.md` に自動保存(並行セッションが衝突しない)。新鮮な共有 `handoff.md` があれば作らない。`CLAUDE_HANDOFF_AUTOSTUB=0` で無効化 |
...
│   ├── rules/
│   │   └── cost-optimization.md # グローバル方針: トークン倹約の行動規範(~/.claude/rules/ = 全プロジェクトに常駐。CLAUDE.md は触らない)
...
1. `install.sh` が `home/` の内容を `~/.claude/` に導入します(既存ファイルは `~/.claude/backup-<日時>/` に退避)。**あなたが編集したファイルは上書きしません** — フック・statusline・スキル・agents・output-styles・rules は「マニフェスト方式」で管理し、あなたが手を加えていないファイルだけ自動更新、編集したファイルは保持してテンプレ新版を `<file>.claude-settings-new` として隣に置きます(差分を見て必要なものだけ反映。全部テンプレに合わせるなら `CLAUDE_INSTALL_FORCE=1`)。`settings.json` は専用マージで扱います(下記)。
2. **グローバル方針(トークン倹約の行動規範)は `~/.claude/rules/cost-optimization.md` に置きます**。`~/.claude/rules/*.md` は全プロジェクトのセッションに常駐する公式のユーザールール機構で、`CLAUDE.md` と同じ扱いで読み込まれます。**あなたの `~/.claude/CLAUDE.md` には一切書き込みません**(完全にあなたの自由記述領域)。旧版が `CLAUDE.md` に埋め込んでいた管理ブロックが残っている場合は、二重ロード(常駐トークンの二重計上)を避けるため、そのブロックだけを除去します(ブロック外のあなたの記述は保持)。
3. 既存の `settings.json` がある場合は**保持マージ**します。**既定は「あなたの既存値を優先」** — テンプレートは“守り”(hooks・deny リスト・予算/上限 env・statusline)を追加し、あなたがまだ設定していないキーを補うだけで、あなたが明示した値(`model`・`outputStyle`・予算 env・認証など)は**勝手に書き換えません**。例えば、あなたが選んだ `model`(上位/最新モデル)や `outputStyle: terse`(簡潔=節約系)がインストールで下位・標準へ差し替えられることはありません。
4. **推奨値との食い違いは明示**します(黙って上書きしない透明性): `model: 既存 "opus" を維持(テンプレ推奨: "sonnet")` のように、テンプレ推奨と異なる項目を1件ずつ表示。積極的にテンプレ推奨へ寄せたいときだけ `CLAUDE_INSTALL_PREFER=template` を付けて再実行すると、`model=sonnet` などコスト最適化寄りの値に切り替わります(`outputStyle` は `Explanatory`/`Learning` のような冗長系のみ標準へ戻し、`terse` 等は温存)。
5. Claude Code を起動し、`/cost-audit` を実行して現状の監査レポートを確認します。
...
- 更新された `home/` のファイル(フック・スキル・statusline 等)を `~/.claude/` へ反映します。**内容が同一のファイルはスキップ**、**あなたが編集したファイルは上書きせず保持**(新版は `<file>.claude-settings-new` に)、未編集のファイルだけ自動更新します。集計を `更新 N / 変更なし M / 保持 K` で表示します(マニフェスト `~/.claude/.claude-settings-manifest` で「前回入れた版のまま=未編集」を判定)。
- 変更のあった既存ファイルは `~/.claude/backup-<日時>/` に退避してから上書きします。
- `settings.json` は**あなたの既存値を優先してマージ**され、あなたが加えた設定(`model`・`outputStyle`・env・認証・独自 permissions/hooks)は保たれます。**マージ結果が既存と同一なら書き換えず**「変更なし・スキップ」と表示します(不要な再書き込み・再バックアップをしません)。テンプレ推奨と食い違う項目は、**`キー: 既存 X を維持(テンプレ推奨: Y)` の形で1件ずつ表示**します(例: `env.CLAUDE_SESSION_BUDGET_USD: 既存 "20" を維持(テンプレ推奨: "5")`)。あなたのコスト関連設定が黙って書き換わることはありません。推奨値へ寄せたいときだけ `CLAUDE_INSTALL_PREFER=template` を付けて再実行します(そのときは `キー: X → Y(推奨を適用)` と表示。不足キーの補完は件数で要約)。
- グローバル方針は `~/.claude/rules/cost-optimization.md` として更新されます(マニフェスト方式=あなたが編集していなければ自動更新、編集していれば保持)。**あなたの `~/.claude/CLAUDE.md` は一切触りません**。
