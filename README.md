...
表示例:
```
🤖 Sonnet │ 💰 $2.50/$5 █████░░░░░ 50% │ 🧠 60k/120k █████░░░░░ 50% │ 🎯 10T:$0.65 │ 🔄 7 │ 💾 90% │ ✍️ terse │ 📝 +12/-3
```

| 項目 | 意味 | 色・絵文字の変化 |
|---|---|---|
| 🤖 モデル名 | 現在のモデル | — |
| 💰 予算バー | セッションコスト / 上限。`$使用額/$上限 バー 使用率%` | 通常 💰 → 80%で ⚠️ → 超過で 🛑(バーは満杯・%は実値で超過分がわかる)。バー色 緑→黄→赤 |
| 🧠 ctxバー | 現在のコンテキスト / 肥大ガード閾値。`現在k/上限k バー 使用率%` | 90%超・200k超で 🔴。「あと何%で肥大ガードが介入するか」の目安 |
...
閾値は `~/.claude/settings.json` の env(`CLAUDE_SESSION_BUDGET_USD` / `CLAUDE_CTX_LIMIT_TOKENS` / `CLAUDE_TURN_BUDGET_USD`)に連動します。

> **effort(low/high 等)は表示できません。** Claude Code が statusline に渡す情報にも、セッションのトランスクリプトにも effort は記録されておらず(実データで確認済み)、取得手段がないためです。effort はハーネスが内部管理するパラメータで、フック・スクリプト・statusline のいずれからも読めません。代わりに、動作中の出力スタイル(モード)を ✍️ で表示しています。
